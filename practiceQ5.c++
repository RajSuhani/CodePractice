...
public:
    ...
    int find_rightmost_prefix(int target = 0) {
        ...
        auto exist = [&](int node_, int sum_before_) -> bool {
            return mn[node_] <= target - sum_before_ &&
                   target - sum_before_ <= mx[node_];
        };

        if (!exist(node, sum_before))
            return -1;
        ...
        while (l != r) {
            ...
            if (exist(rchild, sum_before_right)) {
                node = rchild;
                l = m + 1;
